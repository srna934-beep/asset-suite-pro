...
const FIELDS: FieldDef[] = [
  ...
  { name: "plate_number", label: "رقم اللوحة" },
  { name: "chassis_number", label: "رقم الهيكل" },
  { name: "driver_name", label: "اسم السائق" },
  { name: "driver_phone", label: "جوال السائق" },
...
const INV = [["vehicles-list"], ["dashboard-totals"]];
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-right text-sm">
            <thead><tr className="bg-muted/40 text-[12px] font-bold text-muted-foreground">
              ...
              <th className="px-4 py-3">الماركة/الموديل</th><th className="px-4 py-3">السائق</th>
              <th className="px-4 py-3">انتهاء التأمين</th><th className="px-4 py-3">انتهاء الاستمارة</th>
            ...
            <tbody>
              {filtered.map((v: any) => (
                <tr key={v.id} className="border-t border-border hover:bg-muted/40">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">{v.license_expiry ?? "—"}</td>
              ...
              {filtered.length === 0 && <tr><td colSpan={9} className="py-12 text-center text-muted-foreground">لا توجد مركبات. أضف أول مركبة باستخدام الزر أعلاه.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
